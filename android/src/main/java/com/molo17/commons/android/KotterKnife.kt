...
fun <V : View> ViewHolder.bindView()
    : ReadOnlyProperty<ViewHolder, V> = required(viewFinder, contextFinder)
...
private val ViewHolder.viewFinder: ViewHolder.(Int) -> View?
    get() = { this.itemView.findViewById(it) }
private val ViewHolder.contextFinder: ViewHolder.() -> Context
...
private fun <T, V : View> required(
    finder: T.(Int) -> View?,
    contextFinder: T.() -> Context
): ReadOnlyProperty<T, V> = Lazy { t: T, desc: KProperty<*> ->
    findWithNoId<T, V>(t, desc, finder, contextFinder)
        ?: viewNotFound(0, desc)
}

@Suppress("UNCHECKED_CAST")
fun <T, V> findWithNoId(
    t: T,
    desc: KProperty<*>,
    finder: T.(Int) -> View?,
    contextFinder: T.() -> Context
): V? {
    val ctx = t.contextFinder()
    val id = ctx.resources.getIdentifier(desc.name, "id", ctx.packageName)
    return t.finder(id) as V?
}

// Like Kotlin's lazy delegate but the initializer gets the target and metadata passed to it
private class Lazy<T, V>(
    private val initializer: (T, KProperty<*>) -> V
) : ReadOnlyProperty<T, V>, LifecycleEventObserver {
    ...
    override fun onStateChanged(source: LifecycleOwner, event: Lifecycle.Event) {
        // When the owner of the view gets destroyed, clear the reference to the view.
        // Remove also the observer, which will be re-attached if needed when accessing
        // to this delegate.
